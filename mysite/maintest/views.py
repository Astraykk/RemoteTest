...
class MainTest(object):
	...
	def check(self, request):
		query = request.GET
		path = query.get('path', self.directory)
		print('path=', path)
	...
	def treeview_ajax(self, request):
		...
		query_flag = query.get('flag', '')
		path = os.path.join(DIRECTORY, query_dir)
		#update: do not change root directory of the page here
	...
	def edit_file(self, file_path):
		...
		if os.path.isfile(file_path):
			with open(file_path, 'rb+') as f:
				content = f.read()
				if os.path.splitext(file_path)[1] == '.ptn' or os.path.splitext(file_path)[1] == '.trf':
					print('ok')
					content = binascii.hexlify(content)
					print(len(content))
	...
	def index(self, request):
		...
		query_path = query.get('path', '')
		print('project=', self.directory)
		#self.prevdirectory = self.directory
		newdirectory = os.path.join(DIRECTORY, query_path)
		if self.directory != newdirectory:
			#entering a new project
			self.directory= newdirectory
			#reset stream_status
			self.stream_status[0][1] = UNDONE
			self.stream_status[1][1] = UNDONE
			self.stream_status[2][1] = UNDONE
